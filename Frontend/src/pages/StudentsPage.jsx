...
import { GraduationCap, Search, Filter, UserCheck, CalendarDays, ExternalLink, MoreVertical } from 'lucide-react';
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Manage student profiles, status and counselling</p>
                    </div>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {loading ? (
                                <tr><td colSpan="6" className="px-6 py-10 text-center text-gray-500">Loading directory...</td></tr>
                            ) : students.length === 0 ? (
                                <tr><td colSpan="6" className="px-6 py-10 text-center text-gray-500">No students found.</td></tr>
                            ) : students.map((std) => (
                                <tr key={std._id} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="font-semibold text-gray-800">{std.fullName}</div>
                                        <div className="text-xs text-gray-400 mt-0.5">{std.email}</div>
                                        <div className="text-[10px] text-gray-400">{std.phone}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-medium">{std.educationHistory?.[0]?.qualification || 'N/A'}</div>
                                        <div className="text-[10px] text-gray-400 uppercase mt-1">Scores: {std.testScores?.overall || 'N/A'}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4">
                                        <select
                                            ...
                                            className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase border-none focus:ring-0 cursor-pointer ${std.status === 'prospect' ? 'bg-blue-100 text-blue-600' :
                                                    std.status === 'enrolled' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'
                                                }`}
                                        >
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <button className="p-1.5 text-gray-400 hover:text-blue-500 transition-colors" title="View Profile">
