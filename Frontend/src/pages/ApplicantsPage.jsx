...
import { BookMarked, Search, Filter, Mail, Plane, Plus, CheckCircle, FileText } from 'lucide-react';
        ...
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">University Applications</h1>
                        <p className="text-sm text-gray-500">Track student applications and visa processes</p>
                    </div>
                </div>
                <button
                            ...
                            {loading ? (
                                ...
                                <tr key={app._id} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="font-semibold text-gray-800">{app.studentId?.fullName || 'Unknown Student'}</div>
                                        <div className="text-xs text-blue-600 font-medium mt-0.5 uppercase tracking-tighter">
                                            {app.universityName} ({app.country})
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-medium">{app.courseName}</div>
                                        <div className="text-[10px] text-gray-400 uppercase mt-1">Intake: {app.intake}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm">App Fee: {app.applicationFee?.amount} {app.applicationFee?.currency}</div>
                                        <div className="text-xs text-gray-400">Paid: {app.applicationFee?.status === 'paid' ? 'Yes' : 'No'}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${app.status === 'offer-received' ? 'bg-green-100 text-green-600' :
                                                app.status === 'draft' ? 'bg-gray-100 text-gray-600' : 'bg-blue-100 text-blue-600'
                                            }`}>
                                            {app.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => updateStatus(app._id, 'offer-received')}
                                                className="p-1.5 text-green-500 hover:bg-green-50 rounded" title="Mark Offer Received"
                                            >
                                                <Mail size={16} />
                                            </button>
                                            <button
                                                onClick={() => updateStatus(app._id, 'visa-applied')}
                                                className="p-1.5 text-blue-500 hover:bg-blue-50 rounded" title="Mark Visa Applied"
                                            >
                                                <Plane size={16} />
                                            </button>
                                        </div>
