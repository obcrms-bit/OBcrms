    ...
    static async generateInvoicePDF(invoice, student, company) {
        return new Promise((resolve, reject) => {
            try {
                ...
                // Header
                doc.fillColor("#444444").fontSize(20).text(company.name || "Trust Education", 50, 50);
                ...
                doc.fontSize(10).text(`Invoice Number: ${invoice.invoiceNumber}`, 50, 185, { align: "right" });
                doc.text(`Date: ${new Date().toLocaleDateString()}`, 50, 200, { align: "right" });
                doc.text(`Due Date: ${new Date(invoice.dueDate).toLocaleDateString()}`, 50, 215, { align: "right" });
                ...
                items.forEach((item) => {
                    ...
                    doc.text(`${invoice.currency} ${Number(item.amount).toFixed(2)}`, 400, position, { align: "right" });
                    position += 20;
                });
                ...
                doc.text(`${invoice.currency} ${Number(invoice.subTotal).toFixed(2)}`, 400, totalTop + 10, { align: "right" });

                doc.text(`Tax (${invoice.tax?.percentage || 0}%)`, 350, totalTop + 25);
                doc.text(`${invoice.currency} ${Number(invoice.tax?.amount || 0).toFixed(2)}`, 400, totalTop + 25, { align: "right" });

                doc.font("Helvetica-Bold").fontSize(12).text("Total", 350, totalTop + 45);
                doc.text(`${invoice.currency} ${Number(invoice.totalAmount).toFixed(2)}`, 400, totalTop + 45, { align: "right" });

                // Footer
                doc.font("Helvetica").fontSize(10).text("Thank you for your business!", 50, 700, { align: "center", width: 500 });
